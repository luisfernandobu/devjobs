        ...
        @if (count($vacantes))
            @foreach ($vacantes as $vacante)
                ...
                <div class="flex flex-col md:flex-row gap-3 items-stretch md:items-center mt-5 md:mt-0">
                    <a href="{{ route('candidatos.index', $vacante) }}" class="bg-slate-800 py-2 px-4 rounded-lg text-white text-xs font-bold uppercase text-center">{{ $vacante->candidatos->count() }} @choice('Candidato|Candidatos', $vacante->candidatos->count())</a>
